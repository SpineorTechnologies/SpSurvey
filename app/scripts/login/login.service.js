...
.factory('loginService', ['envsetting','$http', '$localStorage','$location', function( envsetting,$http, $localStorage, $location){
	
	
		var serverURL=envsetting.client.serverUrl;//$location.protocol() + '://'+ $location.host() +':3010';
		//alert(serverURL);
		var configObj = {};
		configObj.sigin = serverURL+"/api/login/signin/";
		configObj.authenticate = serverURL+"/api/login/authenticate/";
		configObj.sendEmail = serverURL+"/api/email/sendemail/";
		configObj.updatepassword = serverURL+"/api/login/update/";
