        ...
        var directive = {
            ...
            template: function(tElement, tAttrs) {
                var isClickable = angular.isDefined(tAttrs.onClick),
                    clickAttr = isClickable ? 'ng-click="onClick({word : entry.word, size : entry.size})"' : '',
                    text = '{{entry.word}}',
                    size = '{{entry.size}}';
                ...
                return '<div class="tangcloud">' +
                    '<span ng-repeat="entry in words" class={{entry.color}} ' + clickAttr + ' title='+size+' ng-bind-html="entry.word">'+ text +'</span>' +
                    '</div>';
            },

            compile: function(elem) {
                elem.children().children()
                    .addClass('tangcloud-item-' + $interpolate.startSymbol() + 'entry.size>=10?10:entry.size' + $interpolate.endSymbol())
                ...
                return function(scope, elem) {
                    scope.$watch('words', function() {
                        ...
                        function moveSpotOnSpiral(spot, angleMultiplier) {
                            ...
                            spot.startX = scope.centerX + (4* angle) * Math.cos(angle) - (spot.width)+20;
                            spot.startY = scope.centerY + angle * Math.sin(angle*4) - (spot.height)+20;
                            spot.endX = spot.startX + spot.width+20;
                            spot.endY = spot.startY + spot.height+20;
                            return spot;
                        }
